...
| 5 | Retrieval | `enricher.py` (GitHub API → `github_artifacts`); `embedder.py` (chunk 2000-char/200-overlap → `text-embedding-3-small` → pgvector); HNSW index; hybrid vector+BM25 via RRF | ✅ |
| 6 | Investigator agent | `agent.py`: LangGraph ReAct loop; tools `run_sql` / `find_slow_prs` / `semantic_search` (BM25+vector RRF) / `get_release_notes`; budget 8 calls / 90s; `sanitize_messages` fix; `generate_report` retry on validation error; Pydantic `InvestigationReport` output contract | ✅ |
| 7 | Eval | `eval/golden_cases.json` (15 cases: 13 pr_slowdown, 1 star_spike, 1 vllm_pr_slowdown); `eval/prepare.py` (enrich + embed all windows, now uses GitHub search backfill); `eval/evaluator.py` (citation coverage, budget, latency, confidence, optional LLM groundedness judge). **Baseline: 10/10 budget, 10/10 latency, 13.6s avg** | ✅ |
...
| 10 | Harden | Citation coverage fix (enricher GitHub search backfill + SQL citation URL guidance); BM25 hybrid retrieval (RRF, GIN index); expand golden set 10→15 cases; Airflow daily pipeline (`dags/repopulse_daily.py`); `download_range` / `process_range` programmatic APIs | ✅ |
...
**Still open:**
- ~~**Citation coverage gap**~~ → resolved: 0.08 → **0.83 avg** (15-case benchmark, June 2026). Fixed via `find_slow_prs` tool (queries `github_artifacts` with GitHub API timestamps, catches pre-window PRs), GitHub search API backfill in enricher, mandatory semantic_search step, `generate_report` retry.
- ~~**Hybrid (BM25) retrieval**~~ → resolved: `semantic_search` now uses PostgreSQL `websearch_to_tsquery` + `ts_rank` fused with vector search via RRF (k=60). GIN index in `infra/05_add_bm25_index.sql`; apply with `make migrate-bm25`.
...
- **2026-06-01** — Hybrid BM25 retrieval (Slice 10): `semantic_search` now does RRF fusion of (a) vector cosine search and (b) PostgreSQL `websearch_to_tsquery` + `ts_rank`. Each path de-duplicates to one result per artifact via `DISTINCT ON` before ranking. RRF constant k=60. GIN index in `infra/05_add_bm25_index.sql`; apply with `make migrate-bm25`. Query falls back gracefully to vector-only when BM25 finds no matches.
- **2026-06-01** — Airflow daily pipeline (Slice 10): `dags/repopulse_daily.py` defines `repopulse_daily` DAG (schedule `0 6 * * *`, `catchup=False`, `max_active_runs=1`). Seven tasks: download → process → detect → enrich → embed → investigate → export. All tasks use `PythonOperator` calling programmatic APIs extracted from existing CLI modules (`download_range`, `process_range`). `MAX_INVESTIGATIONS_PER_RUN=5` caps daily LLM cost. Airflow uses the same Python venv as repopulse; metadata DB defaults to SQLite at `.airflow/`. Setup: `make airflow-init && make airflow-up`. UI at http://localhost:8080.
- **2026-06-01** — Citation coverage fix (Slice 10): enricher now calls GitHub search API (`/search/issues?type:pr+merged:start..end`) alongside the events-table query, so PRs opened before the events window but merged in the anomaly window are fetched and embedded. `Citation.source_url` guidance updated to use GitHub URLs for individual PRs even when found via SQL.
- **2026-06-01** — Added `find_slow_prs` tool to agent (Slice 10): queries `github_artifacts` directly using GitHub API timestamps (`closed_at - created_at`). Fixes citation coverage for anomalies driven by PRs opened before the GH Archive event window (e.g. dbt PR #9839 open 373 days). GH Archive self-joins only work when both open and close events are in our window; `github_artifacts` always has the full age from GitHub API.
- **2026-06-01** — Citation coverage 0.08 → 0.83 (15-case benchmark). Combination of `find_slow_prs` tool, mandatory semantic_search in investigation sequence, `generate_report` retry on Pydantic validation error, and golden case URL expansion (added symptom PRs as alternative valid evidence alongside root-cause PRs).
- **2026-06-01** — Budget enforcement fix: `should_continue` now checks `tool_calls_used + len(pending_calls) > MAX_TOOL_CALLS` instead of `>= MAX_TOOL_CALLS` after execution. Prevents batch tool calls from silently exceeding the limit. `MAX_TOOL_CALLS` raised from 6 → 8 to accommodate 4-tool investigation sequence.
- **2026-06-01** — All module paths changed from relative `Path("data")` to absolute `Path(__file__).resolve().parent.parent / "data"`. Required for Airflow (tasks run from a different CWD than the project root). `load_dotenv()` updated to explicit absolute `.env` path for same reason. `.airflow/` added to `.gitignore`.
